...
SessionStore.prototype.verify = function (req, handle, cb) {
    console.log('in session store verify function');
    ...
    console.log('about to check key which contains the state of the app that we have to compare against the request');
    console.log(`key is ${this._key}`);
    // console.log('req obj contents');
    // for(const prop of Object.getOwnPropertyNames(req)) {
    //     console.log(`${prop}: ${req[prop]}`);
    // }
    console.log('check url');
    console.log(req.url);
    console.log('check session - this needs to include a session key inside that has a state for the app');
    console.log(JSON.stringify(req.session, null, 2));

    const key = this._key;
    if (!req.session[key]) {
        console.log('about to enter initial session key callback func');
    ...
    if (!state) {
        console.log('about to enter state callback func');
        return cb(null, false, {
            message: "Unable to verify authorization request state.",
        });
    }
    ...
    console.log('about to do the state vs local handle comparison');

    if (state.handle !== handle) {
        console.log(`state.handle ${state.handle} did not match handle ${handle} so doing the callback with an error msg`);
        return cb(null, false, {
    ...
    console.log('doing the session verify callback');
