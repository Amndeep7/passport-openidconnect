...
Strategy.prototype.authenticate = function (req, options) {
    console.log('in strategy authenticate function');
    options = options || {};
    const self = this;
    ...
    if (req.query && req.query.code) {
        console.log('forming the /token request using the response from /authorize via the auth code grant flow - this is nominally the second invocation of the function');
        ...
        function restored(err, ctx, state) {
            console.log('entered the session verify callback');
            if (err) {
            ...
            if (!ctx) {
                console.log('had no context so failing with a 403');
            ...
            if (self._pkce) {
                if (!ctx.verifier) {
                    console.log('if pkce was enabled and no verifier was found in the context, we fail out');
                    return self.error(
                        new Error(
            ...
            console.log('about to try to get the oauth access token');
            self._oauth2.getOAuthAccessToken(
                code,
                params,
                function (err, accessToken, refreshToken, params) {
                    console.log('entered the callback for the oauth access token request');
                    if (err) {
                        console.log('immediate error out');
                        console.dir(err, { depth: null });
                    ...
                    console.log('claims', claims.iss);
                    console.log('self', self._issuer);
        ...
        console.log('got state from the request - about to verify it');
        console.dir(state, { depth: null });
        try {
            self._stateStore.verify(req, state, restored);
        } catch (ex) {
            console.log('errored out in verifying the state');
            console.dir(ex, { depth: null });
            return self.error(ex);
        }
    } else {
        console.log('forming the /authorize request - this is nominally the first invocation of the function');
